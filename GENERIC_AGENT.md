...
- `lumio_repositories` - Repository GitHub censiti (user_id, github_owner, github_repo, access_token, docora_repository_id, sync_status, sync_error_message, sync_failed_at, last_commit_hash, last_commit_at, last_sync_added, last_sync_updated, last_sync_removed, last_sync_unchanged)
...
**Note Milestone v1.7 - Sync Recovery:**
- Colonna `sync_error_message` (text, nullable) aggiunta a `lumio_repositories` per messaggio errore sync
- Colonna `sync_failed_at` (timestamptz, nullable) aggiunta a `lumio_repositories` per timestamp fallimento
- Vincolo `sync_status` aggiornato: aggiunto `'sync_failed'` ai valori ammessi
...
| `/delete` | File rimosso |
| `/sync_failed` | Sync repository fallito (errore git) |
